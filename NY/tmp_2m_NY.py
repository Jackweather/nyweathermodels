...
# Dynamically locate the HRRR_NY/tmp_2m_NY base folder
def find_hrrr_ny_base_dir(start_dir):
    while True:
        if os.path.basename(start_dir) == "tmp_2m_NY" and os.path.basename(os.path.dirname(start_dir)) == "HRRR_NY":
            return start_dir
        parent_dir = os.path.dirname(start_dir)
        if parent_dir == start_dir:  # Reached the root directory
            raise RuntimeError("HRRR_NY/tmp_2m_NY base folder not found.")
        start_dir = parent_dir

# Set the base directory for HRRR_NY/tmp_2m_NY output
current_dir = os.path.dirname(os.path.abspath(__file__))
BASE_DIR = find_hrrr_ny_base_dir(current_dir)
