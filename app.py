...
def get_latest_saved_dir():
    # Find the latest yymmdd/run_hour directory in /var/data
    if not os.path.exists(BASE_DIR):
        return None
    yymmdd_dirs = sorted([d for d in os.listdir(BASE_DIR) if d.isdigit() and len(d) == 6], reverse=True)
    for yymmdd in yymmdd_dirs:
        yymmdd_path = os.path.join(BASE_DIR, yymmdd)
        if not os.path.isdir(yymmdd_path):
            continue
        hour_dirs = sorted([h for h in os.listdir(yymmdd_path) if h.isdigit() and len(h) == 2], reverse=True)
        for hour in hour_dirs:
            hour_path = os.path.join(yymmdd_path, hour)
            if os.path.isdir(hour_path):
                return hour_path
    return None

# Endpoint to list saved PNGs in the latest run folder
@app.route("/list_saved_pngs")
def list_saved_pngs():
    latest_dir = get_latest_saved_dir()
    if not latest_dir:
        return jsonify([])
    png_files = sorted([f for f in os.listdir(latest_dir) if f.lower().endswith('.png')])
    # Serve via /get_saved_png/<filename>
    return jsonify([f"/get_saved_png/{f}" for f in png_files])

# Endpoint to serve a saved PNG from the latest run folder
@app.route("/get_saved_png/<filename>")
def get_saved_png(filename):
    latest_dir = get_latest_saved_dir()
    if not latest_dir:
        return jsonify({"error": "No saved PNGs found"}), 404
