...
# Set current directory and base directory for HRRR output
current_dir = os.path.dirname(os.path.abspath(__file__))
BASE_DIR = os.path.join(current_dir, "mslp_prate_csnow_NY")

# Make sure subfolders exist (recreate grib cleanly)
grib_dir = os.path.join(BASE_DIR, "grib")
...
processed_steps_file = os.path.join(BASE_DIR, "processed_steps.txt")
...
for step_group in forecast_steps:
    ...
    # Download GRIB files for the group
    for step in step_group:
        mslp_gribs.append(get_hrrr_grib(step, "MSLMA"))
        prate_gribs.append(get_hrrr_grib(step, "PRATE"))
        csnow_gribs.append(get_hrrr_grib(step, "CSNOW"))
        cfrzr_gribs.append(get_hrrr_grib(step, "CFRZR"))
        cicep_gribs.append(get_hrrr_grib(step, "CICEP"))

    # Process each step in the group
    for i, step in enumerate(step_group):
        if mslp_gribs[i] and prate_gribs[i]:
            plot_combined(
                mslp_gribs[i],
                prate_gribs[i],
                step,
